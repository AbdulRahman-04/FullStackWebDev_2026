...
// loadd env variables in config 
func LoadEnv() {
	err := godotenv.Load()
	if err != nil {
		log.Fatalf("couldn't load env in config")
	}
	...
	if cfg.DB_URL == "" || cfg.Port == "" {
		log.Fatalf("db url or port no missing in env")
	}

	if cfg.JWT_KEY == "" || cfg.JWT_REFRESH_KEY == "" {
		log.Fatalf("jwt key or refresh key missing in env")
	}

	if cfg.GroqApiKey == "" || cfg.GroqApiURL == "" {
		log.Fatalf("groqApiKey or GroqApiUrl missing in env")
	}
